...
structure ExternalPngMultiIdatPhysSpec (px : Type u) [PixelFormat px] [Png.PixelFormat px] where
  bitmap : Bitmap px
...
lemma expectedMetadata_srgb_none (s : ExternalPngMultiIdatPhysSpec px) :
  ...
  unfold MultiIdatPhysContainerSpec.expectedMetadata
    MultiIdatPhysContainerSpec.toGeneric
    MultiIdatGenericPreChunkContainerSpec.expectedMetadata
  rcases s.container.pHYs with _ | _ <;> rfl
