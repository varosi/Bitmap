import Bitmap.Lemmas.Png.MultiIdatPreChunkContainerSpec

namespace Bitmaps

namespace Lemmas

open Png

/-! ## bKGD as an instance of the generic pre-chunk container spec (Step 2d) -/

/-- A `bKGD` chunk witness, parameterised by the surrounding PNG header. -/
structure BkgdChunkWitness (hdr : PngHeader) where
  payload : ByteArray
  bkgd : PngBackground
  hParses : parseBkgdData hdr payload = some bkgd
  hPayloadFits : payload.size < 2 ^ 32

/-- Build a `PreIdatChunk` from a `BkgdChunkWitness`. -/
def BkgdChunkWitness.toPreIdat {hdr : PngHeader} (w : BkgdChunkWitness hdr) :
    PreIdatChunk hdr where
  typeBytes := bkgdTypeBytes
  hTypeSize := by rfl
  hTypeNotIDAT := by decide
  hTypeNotIHDR := by decide
  payload := w.payload
  hPayloadFits := w.hPayloadFits
  expectedMetadata := { PngMetadata.empty with background := some w.bkgd }
  hAcceptStep := by
    intro fuel bytes pos state hpos hLen hread hheader hPLTE hIDAT _hClosed hMetaEmpty
    have hNotIHDR : (bkgdTypeBytes == ihdrTypeBytes) = false := by decide
    have hNotPLTE : (bkgdTypeBytes == plteTypeBytes) = false := by decide
    have hNotIDAT : (bkgdTypeBytes == idatTypeBytes) = false := by decide
    have hNotIEND : (bkgdTypeBytes == iendTypeBytes) = false := by decide
    have hNotTRNS : (bkgdTypeBytes == trnsTypeBytes) = false := by decide
    have hIsBKGD : (bkgdTypeBytes == bkgdTypeBytes) = true := by decide
    have hDup : state.metadata.background.isSome = false := by
      rw [hMetaEmpty]; rfl
    have hStep := parsePngLoopFuelWithMetadata_accepts_bKGD fuel bytes pos state
      hdr bkgdTypeBytes w.payload (pos + 8 + w.payload.size + 4) w.bkgd
      hpos hLen hread hheader
      hNotIHDR hNotPLTE hNotIDAT hNotIEND hNotTRNS
      hIsBKGD hIDAT hDup w.hParses
    rw [hStep]
    show parsePngLoopFuelWithMetadata fuel bytes (pos + 8 + w.payload.size + 4)
      { state with metadata :=
          { state.metadata with background := some w.bkgd } } = _
    congr 1
    rw [hMetaEmpty]

/-- A PNG byte stream with the multi-IDAT shape plus an optional `bKGD`
chunk between IHDR and the first IDAT. -/
structure MultiIdatBkgdContainerSpec where
  header : PngHeader
  idatChunks : List ByteArray
  hChunkSize : ∀ c, c ∈ idatChunks → c.size < 2 ^ 32
  hNonempty : idatChunks ≠ []
  hBitDepth : header.bitDepth = 8 ∨ header.bitDepth = 16
  hColorType :
    header.colorType = 0 ∨ header.colorType = 2 ∨
      header.colorType = 4 ∨ header.colorType = 6
  hInterlace : header.interlace = 0
  hWidth : header.width < 2 ^ 32
  hHeight : header.height < 2 ^ 32
  /-- Optional `bKGD` chunk placed between IHDR and the first IDAT. -/
  bKGD : Option (BkgdChunkWitness header)

namespace MultiIdatBkgdContainerSpec

variable (s : MultiIdatBkgdContainerSpec)

def toGeneric : MultiIdatGenericPreChunkContainerSpec where
  header := s.header
  idatChunks := s.idatChunks
  hChunkSize := s.hChunkSize
  hNonempty := s.hNonempty
  hBitDepth := s.hBitDepth
  hColorType := s.hColorType
  hInterlace := s.hInterlace
  hWidth := s.hWidth
  hHeight := s.hHeight
  preChunk := s.bKGD.map BkgdChunkWitness.toPreIdat

def bytes : ByteArray := s.toGeneric.bytes
def idatData : ByteArray := s.toGeneric.idatData
def expectedMetadata : PngMetadata := s.toGeneric.expectedMetadata
lemma bytes_size_ge_8 : 8 ≤ s.bytes.size := s.toGeneric.bytes_size_ge_8

theorem parsePngForDecode_multiIdatBkgdContainerSpec_correct :
    parsePngForDecode s.bytes s.bytes_size_ge_8 =
      some { header := s.header, idat := s.idatData,
             metadata := s.expectedMetadata } :=
  s.toGeneric.parsePngForDecode_correct

end MultiIdatBkgdContainerSpec

end Lemmas

end Bitmaps
