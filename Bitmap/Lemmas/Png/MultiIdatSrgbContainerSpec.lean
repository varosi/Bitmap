...
The byte layout is
    ...
    signature ++ IHDR ++ (sRGB chunk, if any) ++ IDAT* ++ IEND.
...
/-- A `sRGB` chunk witness: the raw 1-byte payload plus the decoded
`PngSrgbIntent` and the round-trip witness
`parseSrgbData payload = some srgb`. -/
...
/-- The wrapped `sRGB` chunk bytes (12-byte overhead + 1-byte payload), or
the empty byte array when no `sRGB` chunk is present. -/
def srgbChunkBytes : ByteArray :=
  match s.sRGB with
  | none => ByteArray.empty
  | some w => mkChunkBytes srgbTypeBytes w.payload

/-- Wire size of the `sRGB` chunk: 13 bytes when present, 0 otherwise. -/
def sRGBWireSize : Nat :=
  if s.sRGB.isSome then 13 else 0

/-- The byte layout: signature, IHDR, optional sRGB, concatenated IDATs, IEND. -/
def bytes : ByteArray :=
  pngSignature ++
...
def expectedMetadata : PngMetadata :=
  ...
  | some w => { PngMetadata.empty with srgb := some w.srgb }

/-- Bytes layout reduces to the multi-IDAT layout when no `sRGB` is present. -/
lemma bytes_eq_multi_of_none (h : s.sRGB = none) :
    s.bytes = s.toMulti.bytes := by
  unfold bytes toMulti MultiIdatContainerSpec.bytes srgbChunkBytes
  rw [h]
  simp

/-- The expected metadata is empty when no `sRGB` is present. -/
lemma expectedMetadata_of_none (h : s.sRGB = none) :
    s.expectedMetadata = PngMetadata.empty := by
  unfold expectedMetadata; rw [h]

/-! ### Helpers for the `sRGB = some` case -/

/-- If `sRGB = some w`, the witness payload has the canonical 1-byte length. -/
lemma srgbWitness_payload_size {w : SrgbChunkWitness} : w.payload.size = 1 := by
  -- parseSrgbData first rejects size ≠ 7, so the round-trip witness fixes it.
  have h := w.hParses
  unfold parseSrgbData at h
  by_contra hne
  simp [hne] at h

/-- The wrapped `sRGB` chunk bytes have size 13 when `sRGB = some w`. -/
lemma srgbChunkBytes_size_of_some {w : SrgbChunkWitness} (h : s.sRGB = some w) :
    s.srgbChunkBytes.size = 13 := by
  unfold srgbChunkBytes
  rw [h]
  rw [mkChunkBytes_size srgbTypeBytes w.payload (by rfl : srgbTypeBytes.size = 4)]
  rw [srgbWitness_payload_size]

/-- `sRGBWireSize` equals 13 when `sRGB = some w`. -/
lemma sRGBWireSize_of_some {w : SrgbChunkWitness} (h : s.sRGB = some w) :
    s.sRGBWireSize = 13 := by
  unfold sRGBWireSize
  rw [h]
  simp

/-- `sRGBWireSize` equals 0 when `sRGB = none`. -/
lemma sRGBWireSize_of_none (h : s.sRGB = none) :
    s.sRGBWireSize = 0 := by
  unfold sRGBWireSize
  rw [h]
  simp
...
/-- Total bytes size: 8 (sig) + 25 (IHDR) + sRGB (0 or 13) + IDATs + 12 (IEND). -/
lemma bytes_size :
    s.bytes.size =
      45 + s.sRGBWireSize + s.toMulti.idatTotalWireSize := by
  unfold bytes
  have hIhdrSize :
  ...
  have hChrmSize : s.srgbChunkBytes.size = s.sRGBWireSize := by
    unfold sRGBWireSize
    match h : s.sRGB with
    | none => unfold srgbChunkBytes; rw [h]; simp
    | some w =>
        rw [s.srgbChunkBytes_size_of_some h]
        simp
  have hIdatSize :
...
def idatOffsetS (i : Nat) : Nat :=
  33 + s.sRGBWireSize + MultiIdatContainerSpec.idatPrefixWireSize s.idatChunks i

/-- Byte offset of the IEND chunk. -/
def iendOffsetS : Nat :=
  33 + s.sRGBWireSize + s.toMulti.idatTotalWireSize

/-! ### Bytes-extract lemmas (`sRGB = some` case)
...
/-- Slicing past signature + IHDR gives access to the (sRGB ++ IDATs ++ IEND) region. -/
...
/-- sRGB chunk lives at byte 33 with size 13 (when present). -/
lemma bytes_extract_sRGB {w : SrgbChunkWitness} (hw : s.sRGB = some w) :
    s.bytes.extract 33 (33 + 12 + w.payload.size) =
      mkChunkBytes srgbTypeBytes w.payload := by
  rw [srgbWitness_payload_size (w := w)]
  have hChrmChunkSize : s.srgbChunkBytes.size = 13 :=
    s.srgbChunkBytes_size_of_some hw
  have hSizeBound : 33 + 13 ≤ s.bytes.size := by
    rw [bytes_size, s.sRGBWireSize_of_some hw]; omega
  have hSkip := s.bytes_extract_skip_through_ihdr 0 13 hSizeBound
  rw [show (33 : Nat) + 0 = 33 from rfl] at hSkip
  rw [show (33 : Nat) + 13 = 33 + 12 + 1 from rfl] at hSkip
  rw [hSkip]
  rw [byteArray_extract_append_prefix
    (a := s.srgbChunkBytes)
    (b := s.toMulti.idatChunksBytes ++ mkChunkBytes iendTypeBytes ByteArray.empty)
    (n := 13) (by rw [hChrmChunkSize])]
  rw [show (13 : Nat) = s.srgbChunkBytes.size from hChrmChunkSize.symm]
  rw [ByteArray.extract_zero_size]
  unfold srgbChunkBytes
  rw [hw]

/-- Slicing past signature + IHDR + sRGB (when present) reveals the
`(idatChunksBytes ++ IEND)` suffix — identical in shape to the multi-IDAT
case past IHDR. -/
lemma bytes_extract_skip_through_gamma {w : SrgbChunkWitness} (hw : s.sRGB = some w)
    (start finish : Nat) (_h : 46 + finish ≤ s.bytes.size) :
    s.bytes.extract (46 + start) (46 + finish) =
      (s.toMulti.idatChunksBytes ++ mkChunkBytes iendTypeBytes ByteArray.empty).extract
        start finish := by
  have hChrmChunkSize : s.srgbChunkBytes.size = 13 :=
    s.srgbChunkBytes_size_of_some hw
  have h1 : 33 + (13 + finish) ≤ s.bytes.size := by
    rw [bytes_size, s.sRGBWireSize_of_some hw] at _h
    rw [bytes_size, s.sRGBWireSize_of_some hw]
    omega
  have hSkip := s.bytes_extract_skip_through_ihdr (13 + start) (13 + finish) h1
  rw [show (33 : Nat) + (13 + start) = 46 + start from by omega] at hSkip
  rw [show (33 : Nat) + (13 + finish) = 46 + finish from by omega] at hSkip
  rw [hSkip]
  have hExt := ByteArray.extract_append_size_add
    (a := s.srgbChunkBytes)
    (b := s.toMulti.idatChunksBytes ++ mkChunkBytes iendTypeBytes ByteArray.empty)
...
lemma bytes_extract_idat_at_P {w : SrgbChunkWitness} (hw : s.sRGB = some w) (i : Nat)
    (h : i < s.idatChunks.length) :
    s.bytes.extract (s.idatOffsetS i) (s.idatOffsetS (i + 1)) =
      mkChunkBytes idatTypeBytes s.idatChunks[i] := by
  unfold idatOffsetS
  rw [s.sRGBWireSize_of_some hw]
  -- 33 + 13 + prefix i = 46 + prefix i; same for i+1
  rw [show (33 : Nat) + 13 + MultiIdatContainerSpec.idatPrefixWireSize s.idatChunks i =
      46 + MultiIdatContainerSpec.idatPrefixWireSize s.idatChunks i from by omega]
  rw [show (33 : Nat) + 13 + MultiIdatContainerSpec.idatPrefixWireSize s.idatChunks (i + 1) =
      46 + MultiIdatContainerSpec.idatPrefixWireSize s.idatChunks (i + 1) from by omega]
  have hSizeBound :
      46 + MultiIdatContainerSpec.idatPrefixWireSize s.idatChunks (i + 1) ≤ s.bytes.size := by
    rw [bytes_size, s.sRGBWireSize_of_some hw]
    show 46 + _ ≤ 45 + 13 + s.toMulti.idatTotalWireSize
    unfold MultiIdatContainerSpec.idatTotalWireSize
    show 46 + MultiIdatContainerSpec.idatPrefixWireSize s.idatChunks (i + 1) ≤
      45 + 13 + MultiIdatContainerSpec.idatPrefixWireSize s.toMulti.idatChunks
        s.toMulti.idatChunks.length
    ...
    show 46 + MultiIdatContainerSpec.idatPrefixWireSize s.idatChunks (i + 1) ≤
      45 + 13 + MultiIdatContainerSpec.idatPrefixWireSize s.idatChunks s.idatChunks.length
    omega
...
/-- IEND lives at `iendOffsetS s` with size 12 (when sRGB = some). -/
lemma bytes_extract_iend_P {w : SrgbChunkWitness} (hw : s.sRGB = some w) :
    s.bytes.extract s.iendOffsetS (s.iendOffsetS + 12) =
      mkChunkBytes iendTypeBytes ByteArray.empty := by
  unfold iendOffsetS
  rw [s.sRGBWireSize_of_some hw]
  -- 33 + 13 + idatTotalWireSize = 46 + idatTotalWireSize
  rw [show (33 : Nat) + 13 + s.toMulti.idatTotalWireSize =
      46 + s.toMulti.idatTotalWireSize from by omega]
  have hSizeBound : 46 + s.toMulti.idatTotalWireSize + 12 ≤ s.bytes.size := by
    rw [bytes_size, s.sRGBWireSize_of_some hw]; omega
  have hSizeBound' : 46 + (s.toMulti.idatTotalWireSize + 12) ≤ s.bytes.size := by omega
  have hSkip := s.bytes_extract_skip_through_gamma hw
    s.toMulti.idatTotalWireSize (s.toMulti.idatTotalWireSize + 12) hSizeBound'
  rw [show (46 : Nat) + s.toMulti.idatTotalWireSize + 12 =
      46 + (s.toMulti.idatTotalWireSize + 12) from by omega]
  rw [hSkip]
...
/-! ### `readChunk` lemmas (`sRGB = some` case) -/

set_option maxHeartbeats 800000 in
/-- `readChunk` at byte 8 reads the IHDR chunk. -/
lemma readChunk_p_ihdr {w : SrgbChunkWitness} (hw : s.sRGB = some w)
  ...
  have hSize : 8 + 12 + (encodeIHDRData s.header).size ≤ s.bytes.size := by
    rw [bytes_size, s.sRGBWireSize_of_some hw, hIhdrSize]; omega
...
set_option maxHeartbeats 800000 in
/-- `readChunk` at byte 33 reads the `sRGB` chunk (when present). -/
lemma readChunk_p_sRGB {w : SrgbChunkWitness} (hw : s.sRGB = some w)
    (hLen : 33 + 3 < s.bytes.size) :
    readChunk s.bytes 33 hLen = some (srgbTypeBytes, w.payload, 46) := by
  have hPayloadSize : w.payload.size = 1 := srgbWitness_payload_size
  have hSize : 33 + 12 + w.payload.size ≤ s.bytes.size := by
    rw [bytes_size, s.sRGBWireSize_of_some hw, hPayloadSize]; omega
  ...
  have h := MultiIdatContainerSpec.readChunk_at_mkChunkBytes s.bytes 33
    srgbTypeBytes w.payload (by rfl) hPayloadFits (s.bytes_extract_sRGB hw) hSize hLen
  rw [show 33 + 8 + w.payload.size + 4 = 46 from by rw [hPayloadSize]] at h
  exact h

set_option maxHeartbeats 800000 in
/-- `readChunk` at `idatOffsetS s i` reads the i-th IDAT chunk. -/
lemma readChunk_p_idat {w : SrgbChunkWitness} (hw : s.sRGB = some w) (i : Nat)
    (h : i < s.idatChunks.length)
    (hLen : s.idatOffsetS i + 3 < s.bytes.size) :
    readChunk s.bytes (s.idatOffsetS i) hLen =
      some (idatTypeBytes, s.idatChunks[i],
        s.idatOffsetS i + 8 + s.idatChunks[i].size + 4) := by
  have hChunkSize := s.hChunkSize s.idatChunks[i] (List.getElem_mem h)
  have hChunkRangeBound :
      s.idatOffsetS i + 12 + s.idatChunks[i].size ≤ s.bytes.size := by
    unfold idatOffsetS
    rw [bytes_size, s.sRGBWireSize_of_some hw]
    ...
    show 33 + 13 + MultiIdatContainerSpec.idatPrefixWireSize s.idatChunks i + 12 +
      s.idatChunks[i].size ≤ 45 + 13 +
      MultiIdatContainerSpec.idatPrefixWireSize s.toMulti.idatChunks
        s.toMulti.idatChunks.length
    show _ ≤ 45 + 13 +
      MultiIdatContainerSpec.idatPrefixWireSize s.idatChunks s.idatChunks.length
    omega
  have hWrap :
      s.bytes.extract (s.idatOffsetS i) (s.idatOffsetS i + 12 + s.idatChunks[i].size) =
        mkChunkBytes idatTypeBytes s.idatChunks[i] := by
    have hWidth :
        s.idatOffsetS (i + 1) = s.idatOffsetS i + 12 + s.idatChunks[i].size := by
      unfold idatOffsetS
  ...
  exact MultiIdatContainerSpec.readChunk_at_mkChunkBytes s.bytes (s.idatOffsetS i)
    idatTypeBytes s.idatChunks[i] (by rfl) hChunkSize hWrap hChunkRangeBound hLen

set_option maxHeartbeats 800000 in
/-- `readChunk` at `iendOffsetS s` reads the IEND chunk. -/
lemma readChunk_p_iend {w : SrgbChunkWitness} (hw : s.sRGB = some w)
    (hLen : s.iendOffsetS + 3 < s.bytes.size) :
    readChunk s.bytes s.iendOffsetS hLen =
      some (iendTypeBytes, ByteArray.empty, s.bytes.size) := by
  have hSize : s.iendOffsetS + 12 + ByteArray.empty.size ≤ s.bytes.size := by
    unfold iendOffsetS
    rw [bytes_size, s.sRGBWireSize_of_some hw]
    simp; omega
  have hEmptyFits : (ByteArray.empty : ByteArray).size < 2 ^ 32 := by decide
  have h := MultiIdatContainerSpec.readChunk_at_mkChunkBytes s.bytes s.iendOffsetS
    iendTypeBytes ByteArray.empty (by rfl) hEmptyFits (s.bytes_extract_iend_P hw) hSize hLen
  rw [show s.iendOffsetS + 8 + ByteArray.empty.size + 4 = s.bytes.size from ?_] at h
  · exact h
  · unfold iendOffsetS
    rw [bytes_size, s.sRGBWireSize_of_some hw]
    simp; omega

/-! ### Walk lemmas (`sRGB = some` case)
...
(`expectedMetadata` carries the sRGB-derived `gamma`). -/

/-- State after processing the first `i` IDAT chunks: header set, IDAT
accumulated, and metadata = `s.expectedMetadata` (the sRGB-derived
modification gamma). -/
private def stateAfterIdatsMS (i : Nat) : PngMetadataParseState :=
...
private lemma stateAfterIdatsMS_idat_full :
    (s.stateAfterIdatsMS s.idatChunks.length).idat = s.idatData := by
  unfold stateAfterIdatsMS idatData MultiIdatContainerSpec.idatData toMulti
  rw [List.take_length]

/-- The `idat` field accumulates one more chunk at each step. -/
private lemma stateAfterIdatsMS_idat_succ (i : Nat)
    (h : i < s.idatChunks.length) :
    (s.stateAfterIdatsMS (i + 1)).idat =
      (s.stateAfterIdatsMS i).idat ++ s.idatChunks[i] := by
  unfold stateAfterIdatsMS
...
lemma parsePngLoopFuelWithMetadata_walk_idat_step_P
    {w : SrgbChunkWitness} (hw : s.sRGB = some w) (i : Nat)
    (h : i < s.idatChunks.length)
    (fuel : Nat)
    (hRest : parsePngLoopFuelWithMetadata fuel s.bytes (s.idatOffsetS (i + 1))
              (s.stateAfterIdatsMS (i + 1)) =
              some { header := s.header, idat := s.idatData,
                     metadata := s.expectedMetadata }) :
    parsePngLoopFuelWithMetadata (fuel + 1) s.bytes (s.idatOffsetS i)
        (s.stateAfterIdatsMS i) =
      some { header := s.header, idat := s.idatData,
  ...
  have hChunkRangeBound :
    ...
    rw [hChunkEq]
    omega
  have hLen : s.idatOffsetS i + 3 < s.bytes.size := by omega
  have hPos : s.idatOffsetS i + 8 ≤ s.bytes.size := by omega
  have hReadIdat := s.readChunk_p_idat hw i h hLen
  have hNextOffsetEq :
      s.idatOffsetS i + 8 + s.idatChunks[i].size + 4 = s.idatOffsetS (i + 1) := by
    unfold idatOffsetS
    rw [MultiIdatContainerSpec.idatPrefixWireSize_succ s.idatChunks i h]
    omega
  rw [hNextOffsetEq] at hReadIdat
  have hStep := parsePngLoopFuelWithMetadata_idat_appends_when_open fuel s.bytes
    (s.idatOffsetS i) (s.stateAfterIdatsMS i) s.header idatTypeBytes
    s.idatChunks[i] (s.idatOffsetS (i + 1))
    hPos hLen hReadIdat
    (by unfold stateAfterIdatsMS; rfl)
  ...
  rw [hStep]
  have hIdatEq : (s.stateAfterIdatsMS i).idat ++ s.idatChunks[i] =
      (s.stateAfterIdatsMS (i + 1)).idat := by
    rw [s.stateAfterIdatsMS_idat_succ i h]
  have hStateEq :
      ({ header := some s.header
         idat := (s.stateAfterIdatsMS i).idat ++ s.idatChunks[i]
         seenPLTE := (s.stateAfterIdatsMS i).seenPLTE
         seenIDAT := true
         closedIDAT := false
         metadata := (s.stateAfterIdatsMS i).metadata : PngMetadataParseState }) =
      s.stateAfterIdatsMS (i + 1) := by
    rw [hIdatEq]
    unfold stateAfterIdatsMS
    rfl
  rw [hStateEq]
...
lemma parsePngLoopFuelWithMetadata_walk_idats_aux_P
    ...
    parsePngLoopFuelWithMetadata fuel s.bytes (s.idatOffsetS i)
      (s.stateAfterIdatsMS i) =
      some { header := s.header, idat := s.idatData,
             metadata := s.expectedMetadata } := by
  ...
  | zero =>
      ...
      | succ fuel' =>
          have hOffsetEq : s.idatOffsetS i = s.iendOffsetS := by
            unfold idatOffsetS iendOffsetS MultiIdatContainerSpec.idatTotalWireSize
            rw [hin]
            rfl
          rw [hOffsetEq]
          have hLen : s.iendOffsetS + 3 < s.bytes.size := by
            unfold iendOffsetS
            rw [bytes_size, s.sRGBWireSize_of_some hw]
            omega
          have hPos : s.iendOffsetS + 8 ≤ s.bytes.size := by
            unfold iendOffsetS
            rw [bytes_size, s.sRGBWireSize_of_some hw]
            omega
          have hReadIend := s.readChunk_p_iend hw hLen
          have hSeenIDAT : (s.stateAfterIdatsMS i).seenIDAT = true := by
            unfold stateAfterIdatsMS
            simp
            rw [hin]
            ...
            | cons _ _ => simp
          have hHeader : (s.stateAfterIdatsMS i).header = some s.header := rfl
          have hIdatEq : (s.stateAfterIdatsMS i).idat = s.idatData := by
            rw [hin]; exact s.stateAfterIdatsMS_idat_full
          have hMetaEq :
              (s.stateAfterIdatsMS i).metadata = s.expectedMetadata := rfl
          have hRes :=
            MultiIdatContainerSpec.parsePngLoopFuelWithMetadata_iend_success_step
              fuel' s.bytes s.iendOffsetS (s.stateAfterIdatsMS i) s.header
...
set_option maxHeartbeats 1200000 in
/-- Walk past the sRGB chunk: from pos 33 with the "just-after-IHDR" state,
read the sRGB chunk and recurse at pos 46 with metadata updated. -/
lemma parsePngLoopFuelWithMetadata_walk_sRGB_step
    {w : SrgbChunkWitness} (hw : s.sRGB = some w)
    (fuel : Nat)
    (hRest : parsePngLoopFuelWithMetadata fuel s.bytes (s.idatOffsetS 0)
              (s.stateAfterIdatsMS 0) =
              some { header := s.header, idat := s.idatData,
                     metadata := s.expectedMetadata }) :
  ...
  have hReadSrgb := s.readChunk_p_sRGB hw hLen
  -- Apply parsePngLoopFuelWithMetadata_accepts_sRGB.
  ...
  have hStep := parsePngLoopFuelWithMetadata_accepts_sRGB fuel s.bytes 33
    { header := some s.header, idat := ByteArray.empty,
      seenPLTE := false, seenIDAT := false, closedIDAT := false,
      metadata := PngMetadata.empty }
    s.header srgbTypeBytes w.payload 46 w.srgb
    hPos hLen hReadSrgb rfl
    hNotIHDR hNotPLTE hNotIDAT hNotIEND hNotTRNS hNotBKGD hNotGAMA hNotCHRM
    hIsSRGB
    (show ((false : Bool) || (false : Bool)) = false from rfl)
    (show (PngMetadata.empty.srgb.isSome : Bool) = false from rfl)
    rfl rfl
    w.hParses
  rw [hStep]
  have hOffsetEq : (46 : Nat) = s.idatOffsetS 0 := by
    unfold idatOffsetS
    rw [s.sRGBWireSize_of_some hw]
    unfold MultiIdatContainerSpec.idatPrefixWireSize
    simp
  have hStateEq :
      ({ header := some s.header, idat := ByteArray.empty,
         ...
         metadata := { (PngMetadata.empty : PngMetadata) with
           srgb := some w.srgb } } : PngMetadataParseState) =
      s.stateAfterIdatsMS 0 := by
    unfold stateAfterIdatsMS expectedMetadata
    rw [hw]
    simp [List.foldl]
...
walk through IHDR, then sRGB, then all IDATs, then IEND. -/
lemma parsePngLoopFuelWithMetadata_walk_ihdr_step_P
    {w : SrgbChunkWitness} (hw : s.sRGB = some w) (fuel : Nat)
  ...
  refine ⟨by decide, ?_⟩
  -- Now we need to walk from pos 33 with header set, through sRGB and IDATs.
  -- Need fuel ≥ 1 + length + 1 = length + 2.
  cases fuel with
  | zero => omega
  | succ fuel' =>
      have hFuel' : s.idatChunks.length + 1 ≤ fuel' := by omega
      have hWalk := s.parsePngLoopFuelWithMetadata_walk_sRGB_step hw fuel' ?_
      · exact hWalk
...
theorem parsePngForDecode_multiIdatSrgbContainerSpec_correct_of_some
    ...
    parsePngForDecode s.bytes s.bytes_size_ge_8 =
      some { header := s.header, idat := s.idatData,
             metadata := s.expectedMetadata } := by
  -- For length ≥ 1, parsePngSimple sees IHDR + sRGB chunk after IHDR — the
  -- third readChunk in parsePngSimple reads sRGB (not IDAT), so it returns
  ...
  have hSimpleNone : parsePngSimple s.bytes s.bytes_size_ge_8 = none := by
    ...
    -- Walk IHDR + sRGB (which appears at pos 33, but parsePngSimple expects IDAT).
    ...
    have hRead2 := s.readChunk_p_sRGB hw hLen2'
    -- typ2 = srgbTypeBytes ≠ idatTypeBytes ⇒ parsePngSimple returns none.
    have hChrmNeIdat : (srgbTypeBytes != idatTypeBytes) = true := by decide
    have hCT256 : s.header.colorType < 256 := by
      rcases s.hColorType with h | h | h | h <;> rw [h] <;> decide
    have hParseHdr := parseIHDRData_encodeIHDRData s.header
      s.hWidth s.hHeight s.hBitDepth s.hInterlace hCT256
  ...
  -- Fuel sufficient for IHDR + sRGB + n IDATs + IEND.
...
/-! ### Forward correctness — `sRGB = none` case via the multi-IDAT theorem -/

/-- When no `sRGB` chunk is present, the multi-IDAT-with-gamma spec
