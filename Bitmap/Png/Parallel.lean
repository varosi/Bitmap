...
/-- Tail-recursive concatenation worker used by the capacity-aware path. -/
def concatByteArraysWithCapacityAux :
    List ByteArray → ByteArray → ByteArray
  | [], out => out
  | chunk :: chunks, out =>
      concatByteArraysWithCapacityAux chunks (out ++ chunk)
...
def concatByteArraysWithCapacity (chunks : List ByteArray) : ByteArray :=
  ...
  concatByteArraysWithCapacityAux chunks (ByteArray.emptyWithCapacity capacity)

/-- Split list work into deterministic nonempty groups, sized from the same
work-threshold policy that chooses the number of scheduled shards. -/
def listShardGroups {α : Type u} (groupSize : Nat) : List α → List (List α)
  | [] => []
  | x :: xs =>
      let n := Nat.max 1 groupSize
      (x :: xs).take n :: listShardGroups groupSize ((x :: xs).drop n)
termination_by xs => xs.length
decreasing_by
  have hpos : 0 < Nat.max 1 groupSize := by
...
def deflateStoredGroupedParallel
  ...
  if parallel.useParallel blockCount raw.size &&
    ...
    let groupSize :=
      parallelCeilDiv blockCount (parallel.shardCountForWork blockCount raw.size)
    let groups := listShardGroups groupSize ranges
    concatByteArraysWithCapacity <|
      mapListParallel groups fun group =>
        storedDeflateBlockRangeShardBytes group raw
  else
    deflateStored raw
...
/-- Sequential scanner-based stored-only zlib decoder. This is the reference
for proving that sharded payload extraction preserves the scanned bytes. -/
def zlibDecompressStoredScanned
    (data : ByteArray) (hsize : 2 <= data.size) : Option ByteArray := do
  ...
  if hmin : 6 ≤ data.size then
    ...
    let out := concatByteArrays <| ranges.map fun range => range.bytes deflated
    let pos := data.size - 4
  ...
  if hmin : 6 ≤ data.size then
    ...
    let out :=
      if parallel.useParallel rangeCount deflated.size &&
        ...
        let groupSize :=
          parallelCeilDiv rangeCount (parallel.shardCountForWork rangeCount deflated.size)
        let groups := listShardGroups groupSize ranges
        concatByteArraysWithCapacity <|
          mapListParallel groups fun group =>
            storedInflatePayloadRangeShardBytes group deflated
      else
        concatByteArraysWithCapacity <| ranges.map fun range => range.bytes deflated
    let pos := data.size - 4
    have hAdler : pos + 3 < data.size := by
